...
extern "C" fn main() -> ! {
    ...
    {
        // set up the tilemap - fill all 4 screenblocks for 32x32 tile map
        // This creates a 512x512 pixel scrollable area
        for sb_offset in 0..2 {
            let tsb = TEXT_SCREENBLOCKS.get_frame(10 + sb_offset).unwrap();
            for y in 0..20 {
                let row = tsb.get_row(y).unwrap();
                for (x, addr) in row.iter().enumerate().take(30) {
                    // Different tile for each screenblock to see the boundaries
                    let tile_offset = (sb_offset * 64) as u16;
                    let te = TextEntry::new().with_tile(tile_offset + (y * 30 + x) as u16);
                    addr.write(te);
                }
            }
        }
...

    {
        ...
        // Size(1) = 32x32 tiles (512x512 pixels) using 4 screenblocks
        BG0CNT.write(BackgroundControl::new().with_screenblock(10)); // 32x32 tile size
